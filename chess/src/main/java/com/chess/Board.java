 ...
 *
 * Goals: complete, self-contained, optimized hybrid approach suitable for
 * perft correctness tests. This implementation focuses on clarity and
 * correctness while using efficient bitboard techniques.
 *
 * Usage:
 *  - Compile: javac ChessBoard.java
 *  - Run perft from main: java ChessBoard
 *
 * Limitations & notes:
 *  - This is a single-file engine focused on move generation + perft. It
 *    intentionally avoids search heuristics (TT, eval) to stay focused on
 *    generation correctness and speed.
 *  - Magic numbers used are taken from common public sources (CPW-style)
 *    and work for standard 64-square boards.
 */
...
public final class Board {
    ...
    private static final long[][] ROOK_ATTACKS = new long[64][]; // variable sized
    ...
    private static void initKnightKingPawnAttacks() {
        for (int sq = 0; sq < 64; sq++) {
            ...
            KING_ATTACKS[sq] = ka;
            WHITE_PAWN_ATTACKS[sq] = wpa;
            BLACK_PAWN_ATTACKS[sq] = bpa;
        }
    }
    ...
    public List<Move> generateLegalMoves() {
        ...
        while (it.hasNext()) {
            ...
            boolean ok = !isKingAttacked(sideToMove == WHITE ? BLACK : WHITE);
            unmakeMove(m, ud);
            if (!ok) it.remove();
        }
        return moves;
        ...
        if (kings != 0) {
            ...
            if (us == WHITE) {
                if ((castlingRights & 1) != 0) {
                    if (((occupancy[2] & (SQUARE_BB[5]|SQUARE_BB[6]))==0)) out.add(new Move(from,6,KING,-1,false,false,true));
                }
                if ((castlingRights & 2) != 0) { 
                    if (((occupancy[2] & (SQUARE_BB[1]|SQUARE_BB[2]|SQUARE_BB[3]))==0)) out.add(new Move(from,2,KING,-1,false,false,true));
                }
            } else {
                if ((castlingRights & 4) != 0) {
                    if (((occupancy[2] & (SQUARE_BB[61]|SQUARE_BB[62]))==0)) out.add(new Move(from,62,KING,-1,false,false,true));
                }
                if ((castlingRights & 8) != 0) { 
                    if (((occupancy[2] & (SQUARE_BB[57]|SQUARE_BB[58]|SQUARE_BB[59]))==0)) out.add(new Move(from,58,KING,-1,false,false,true));
                }
            }
        }
    }
    ...
    public UndoData makeMove(Move m) {
        ...
        ud.oldEnPassant = enPassantSquare;
        ...
        ud.capturedType = -1; ud.capturedColor = -1; ud.capturedBB = 0L;
        if (m.isEnPassant) {
            int capSq = us == WHITE ? m.to - 8 : m.to + 8;
            long capBB = 1L<<capSq;
            for (int p = 0; p < 6; p++) {
                if ((pieceBB[them*6 + p] & capBB) != 0) { ud.capturedType = p; ud.capturedColor = them; ud.capturedBB = capBB; pieceBB[them*6 + p] &= ~capBB; break; }
            }
            occupancy[them] &= ~capBB; occupancy[2] &= ~capBB;
        } else {
            for (int p = 0; p < 6; p++) {
                if ((pieceBB[them*6 + p] & toBB) != 0) { ud.capturedType = p; ud.capturedColor = them; ud.capturedBB = toBB; pieceBB[them*6 + p] &= ~toBB; break; }
            }
            if (ud.capturedType >= 0) { occupancy[them] &= ~toBB; occupancy[2] &= ~toBB; }
        }
        ...
        if (m.isCastle) {
            if (us == WHITE) {
                if (m.to == 6) { 
                    int rf = 7, rt = 5; ud.rookFrom = rf; ud.rookTo = rt; ud.movedRook = true;
                } else { 
                    int rf = 0, rt = 3; ud.rookFrom = rf; ud.rookTo = rt; ud.movedRook = true;
                }
            } else {
                if (m.to == 62) {
                    int rf = 63, rt = 61; ud.rookFrom = rf; ud.rookTo = rt; ud.movedRook = true;
                } else {
                    int rf = 56, rt = 59; ud.rookFrom = rf; ud.rookTo = rt; ud.movedRook = true;
                }
            }
            if (ud.movedRook) {
                long rfrom = 1L<<ud.rookFrom; long rto = 1L<<ud.rookTo;
                for (int p = 0; p < 6; p++) {
                    if ((pieceBB[us*6 + ROOK] & rfrom) != 0) { pieceBB[us*6 + ROOK] &= ~rfrom; pieceBB[us*6 + ROOK] |= rto; break; }
                }
            }
        }

        if (m.piece == KING) {
            if (us == WHITE) castlingRights &= ~(1|2); else castlingRights &= ~(4|8);
        }
        if (m.piece == ROOK) {
            if (us == WHITE) {
        ...
        if (ud.capturedType == ROOK) {
            if (ud.capturedColor == WHITE) {
    ...
    public void unmakeMove(Move m, UndoData ud) {
        ...
        int us = them ^ 1;
        sideToMove = us; 
        long fromBB = 1L<<m.from; long toBB = 1L<<m.to;
        if (m.promotion >= 0) {
            pieceBB[us*6 + m.promotion] &= ~toBB;
            pieceBB[us*6 + PAWN] |= fromBB;
        } else {
            pieceBB[us*6 + m.piece] &= ~toBB;
            pieceBB[us*6 + m.piece] |= fromBB;
        }
        if (ud.capturedType >= 0) {
            pieceBB[ud.capturedColor*6 + ud.capturedType] |= ud.capturedBB;
        }
        if (m.isCastle && ud.movedRook) {
            long rfrom = 1L<<ud.rookFrom; long rto = 1L<<ud.rookTo;
            pieceBB[us*6 + ROOK] &= ~rto;
            pieceBB[us*6 + ROOK] |= rfrom;
        }
        enPassantSquare = ud.oldEnPassant;
    ...
    public boolean isSquareAttacked(int sq, int byColor) {
        long occ = occupancy[2];
        if (byColor == WHITE) {
            if ((WHITE_PAWN_ATTACKS[sq] & occupancy[WHITE]) != 0) return true;
        } else {
            if ((BLACK_PAWN_ATTACKS[sq] & occupancy[BLACK]) != 0) return true;
        }
    ...
    public static void main(String[] args) {
        ...
        int[] depths = {1,2,3,4,6,6,6,6,6,6};
        for (int d : depths) {
